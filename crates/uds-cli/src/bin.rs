use uds_cli::cli::{UdsCli, UdsCommand, PluginCommand, FsCommand, FirmwareCommand};
...
fn main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        Some(UdsCommand::Init) => {
            uds_cli::commands::run_init()?
        }
        Some(UdsCommand::Devices { scan, watch }) => {
            uds_cli::commands::run_devices(*scan, *watch)?
        }
        ...
        Some(UdsCommand::Doctor) => {
            uds_cli::commands::run_doctor()?
        }
        Some(UdsCommand::Logs { level, lines }) => {
            uds_cli::commands::run_logs(level.as_deref(), *lines)?
        }
        Some(UdsCommand::Monitor) => {
            uds_cli::commands::run_monitor()?
        }
        Some(UdsCommand::Flash { firmware, verify, ota, partition }) => {
        ...
        Some(UdsCommand::Benchmark { kind }) => {
            uds_cli::commands::run_benchmark(kind)?
        }
        Some(UdsCommand::Plugins { command }) => {
            uds_cli::commands::run_plugins(command)?
        }
        Some(UdsCommand::Rpc { method, params, device_id }) => {
            uds_cli::commands::run_rpc(method, params.as_deref(), device_id.as_deref())?
        }
        Some(UdsCommand::Fs { command }) => {
            uds_cli::commands::run_fs(command)?
        }
        Some(UdsCommand::Generate { input, lang, output }) => {
        ...
        Some(UdsCommand::Firmware { command }) => {
            match command {
